...
var rS        = ".raxus-slider";
var number    = 0;
function move( that,pixel ) {that.css( { "-webkit-transform" : "translate3d(" + pixel + ", 0)","-moz-transform" : "translate3d(" + pixel + ", 0)","-o-transform" : "translate3d(" + pixel + ", 0)","-ms-transform" : "translate3d(" + pixel + ", 0)","transform" : "translate(" + pixel + ")" } )}
function raxusSliderGlobal( that,direction ) {
	...
	if( e.find( ".mini-images" ).length ) {
		...
		var thumbPixel,ifDir = e.attr( "data-thumbnail" ) == "left" || e.attr( "data-thumbnail" ) == "right",miniSlide = ifDir ? miniImgsLi.outerHeight( true ) : miniImgsLi.outerWidth( true ),miniSize = miniImgsLi.size() - 1,miniDirSize = ifDir ? e.height() : e.width(),miniShow = miniDirSize / miniSlide,miniSelected = e.find( ".mini-images li.selected" ).index(),margin = ifDir ? 30 : 20,firstDisplay = parseInt( miniImgsLi.eq( 0 +
		                                                                                                                                                                                                                                                                                                                                                                                                                                      number ).index() );
		...
		if( miniSize - 1 >= miniShow ) {
			if( miniSelected == miniImgsLi.slice( -1 ).index() || miniSelected == miniImgsLi.slice( -2 ).index() ) {
				...
				thumbPixel =
					-miniSelected * miniSlide + miniSlide * (miniShow - 1), number = miniSelected - miniShow + 1;
...
function videoId( data ) {
	...
	return { url : RegExp.$2,id : RegExp.$5,site : RegExp.$2 == "youtube.com" || RegExp.$2 == "youtu.be" ? "http://www.youtube-nocookie.com/embed/" : "http://player.vimeo.com/video/" }
}
function dataEqSel() {
...
jQuery( function() {
	...
	jQuery( document ).ready( function() {jQuery( rS + " .slider-relative" ).find( ".slide:eq(0) .text" ).addClass( "left-animated" )} );
	...
	jQuery( rS + "[data-thumbnail]" ).each( function() {
		...
		var miniCalc = thumb == "bottom" ||
		               thumb == "top" ? jQuery( this ).find( ".mini-images-relative" ).outerHeight() : jQuery( this ).find( ".mini-images-relative" ).outerWidth();
	...
	jQuery( rS ).each( function( index ) {jQuery( this ).attr( "tabindex",index )} );
	dataEqSel();
	...
	function autoPlay( time,that ) {
		var playIt = setInterval( function() {raxusSliderGlobal( that.find( ".arrow-r.autoPlayButton" ),"-" )},time );
		...
		that.on( "touchstart",function() {stopPlay()} );
		that.on( "touchstart",function() {startPlay()} );
		that.hover( function() {stopPlay()},function() {
	...
	jQuery( rS + "[data-thumbbgcolor]" ).each( function() {jQuery( this ).find( ".mini-images" ).css( "background-color",jQuery( this ).attr( "data-thumbbgcolor" ) )} );
	jQuery( document ).on( "click",".arrow-r",function() {raxusSliderGlobal( this,"-" )} );
	jQuery( document ).on( "click",".arrow-l",function() {raxusSliderGlobal( this,"+" )} );
	jQuery( document ).on( "mouseup touchend",".dots li, .mini-images:not(.no-transition) li",function() {raxusSliderGlobal( this,"dot" )} );
	var startX,thumb,direction,
	    slideWhere,finishLeft,finishTop,finish = 0;

	function dirAndThumb( that ) {
		var dataThumbnail = jQuery( that ).parents( ".raxus-slider" ).attr( "data-thumbnail" ),dataDirection = jQuery( that ).parents( ".raxus-slider" ).attr( "data-direction" );
	...
	jQuery( rS + " .slider-relative, " + rS + " .mini-images:not(.no-slide)" ).map( function() {
		this.addEventListener( "touchstart",function( event ) {
			if( jQuery( rS + " .slider-relative, " + rS ).attr( "data-direction" ) == "vertical" ) {
				event.preventDefault();
			}
			touching( this );
	...
	jQuery( document ).on( "mouseup touchend",rS + " .slider-relative",function() {
		...
		if( -startX < -100 ) {
			raxusSliderGlobal( thatR,"-" );
		} else if( -startX > 100 ) {
			raxusSliderGlobal( thatL,"+" );
		}
		rePositioning()
	} );
	...
	jQuery( window ).resize( function() {rePositioning()} )
} );
